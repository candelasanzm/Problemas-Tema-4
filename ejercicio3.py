...
def billetes_dict(v : int, c : int) -> dict:
    """ Diccionario para almacenar los datos de un billete (cuanto vale y la cantidad de billetes de ese valor que hay) """
...
def devolver_cantidad_exacta(b : list[dict], D : int) -> tuple[bool, list[int]]:
    ...
    numeroBilletes = len(b) # Número total de tipos de billetes disponibles
    ...
    for i in range(1, numeroBilletes + 1): # Itera sobre los tipos de billetes
        for j in range(D + 1): # Itera sobre las cantidades de los billetes
            matriz[i][j] = matriz[i - 1][j] # Hereda el valor anterior

            for k in range(b[i - 1]['cantidad'] + 1): # Itera sobre la cantidad de billetes disponibles de un tipo concreto
                if k * b[i - 1]['valor'] <= j:
                    matriz[i][j] = min(matriz[i][j], matriz[i - 1][j - k * b[i - 1]['valor']] + k)

    # Verificar si es posible devolver la cantidad D o no
    ...
    for i in range(numeroBilletes, 0, -1): # Recorremos los billetes en orden inverso
...
def test_devolver_cantidad_exacta():
    ...
    billetes3 = [billetes_dict(3, 4), billetes_dict(6, 2)]
    D6 = 5
    resultado6 = (False, [])
